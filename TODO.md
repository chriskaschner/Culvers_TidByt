...
- [ ] **Weather quiz not defaulting** -- quiz-weather-v1.json should load as the default quiz mode but may not be (verify config path order in engine.js and populateVariantSelect logic). Fix and confirm.
- [ ] **Weather quiz icons need real artwork** -- sky-pattern and temperature-band questions currently use plain color circles (swatches), which don't communicate the concept. Need actual representative pixel art sprites: sun/clear sky, storm clouds, sunset, night sky for sky-pattern; snowflake/icicle, leaf/breeze, sunshine, flame/heat for temperature. Color alone is not enough -- the icons should tell you what the option means before you read the text.
