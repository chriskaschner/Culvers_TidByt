...
- [ ] **Flavor asset parity audit + canonicalization (primary)** -- run a full cross-surface review everywhere a flavor appears (Forecast, Radar, Map, Fronts, Quiz results, Widget, Tidbyt, OG/social). For each flavor, compare displayed name, description copy, and visual asset output (cone/marker/card variants); publish mismatch report, then align all surfaces to one canonical source of truth.
  ...
  - [ ] **Phase 1 — add missing profiles** -- add flavor-colors.js profiles for: Brownie Thunder, Peanut Butter Cup, Blackberry Cobbler, Lemon Berry Layer Cake (all HIGH severity, block correct cone rendering everywhere)
  - [ ] **Phase 2 — resolve name collisions** -- pick canonical names and update flavor-matcher.js, planner-shared.js, flavor-archetypes.json: Chocolate Volcano/Oreo Volcano, Georgia Peach/Pecan, Salted Double Caramel Pecan/Salted Caramel Pecan Pie, OREO Cheesecake variants
  - [ ] **Phase 3 — Tidbyt sync** -- after flavor-colors.js is stable, sync tidbyt/culvers_fotd.star FLAVOR_PROFILES dict to match (also affects community PR)
