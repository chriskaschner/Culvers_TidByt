...
- [ ] **Custard Mad Libs** -- fill-in-the-blank custard story generator as a new quiz mode. User supplies flavor vibes (adjectives, moods, settings) through quiz questions; engine assembles a custard-themed short story with their words inserted; story connects to their archetype and nearby store match. Implement as `quiz-madlibs-v1.json` + new `collectAnswers` result type (`madlib`) in engine.js that renders the assembled story card with their archetype cone.
